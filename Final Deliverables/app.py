...
def login():
    ...
    if request.method == 'POST':
        ...
        mail_check_query = "SELECT * FROM " + \
            "".join(tempUserType)+" WHERE EMAIL = ? and PASSWORD=?"
...
def assignagent():
    ...
    if request.method == 'POST':
        ...
        execute(stmt)
        account = fetch_assoc(stmt)
        sendMailThroughSendGrid(
            ticketid, account['EMAIL'], "Assigned", request.form['id'])
        return redirect(url_for('assignticket'))
...
def agenttickets():
    ...
    if request.method == 'POST':
        ...
        account = fetch_assoc(stmt)
        sendMailThroughSendGrid(
            request.form['id'], account['EMAIL'], "Approved", userID)
        print(request.form.items)
...
def register():
    if (request.method == 'POST'):
        l = request.form.to_dict()
        tempUserType = [x for x in l if x in ["customer", "agent", "admin"]]
        mail_check_query = "insert into " + \
            "".join(tempUserType)+" (EMAIL,NAME,PASSWORD) values (?,?,?)"
        stmt = prepare(conn, mail_check_query)
        bind_param(stmt, 1, request.form['email'])
